      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('YOUR LEARNING', style: SwissTypography.display.copyWith(fontSize: 36, color: fg)),
            const SizedBox(height: SwissSpacing.sm),
            Text('Mastery from your real review history — nothing invented.', style: SwissTypography.body.copyWith(color: mutedFg)),
            ...
            progress.when(
              ...
              data: (p) {
                ...
                final value = (p.decks.isNotEmpty && p.totalReviews > 0)
                    ? (p.decks.fold<num>(0, (s, d) => s + d.accuracy * d.reviews) / p.totalReviews).round()
                    : null;
                return SwissCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    Text('${value ?? "—"}%', style: SwissTypography.display.copyWith(fontSize: 48, color: fg)),
                    const SizedBox(width: 16),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text('MASTERED', style: SwissTypography.label.copyWith(color: fg)),
                      const SizedBox(height: 4),
                      Text(value == null ? 'Review a deck to build mastery.' : '$value% average across ${p.decks.length} decks',
                  ...
                  if (weakest != null && weakest.accuracy < 70) ...[
                    ...
                    SwissButton(label: 'Reinforce ${weakest.title}', variant: SwissButtonVariant.secondary, onPressed: () => context.go(AppRoutes.flashcards)),
                  ],
                ]));
              },
            ),
            ...
            progress.when(
              ...
              data: (p) {
                final decks = [...p.decks]..sort((a, b) => a.accuracy.compareTo(b.accuracy));
                if (decks.isEmpty) return Text('No review data yet.', style: SwissTypography.body.copyWith(color: mutedFg));
                return Column(children: [for (final d in decks) ...[
                  SwissNumberedItem(index: d.accuracy, title: d.title, subtitle: '${d.reviews} reviews'),
                  const SwissHairline(),
                ]]);
              },
            ),
            const SizedBox(height: SwissSpacing.xxl),
            Text('$sourceCount sources · ${aiUsed ?? "—"} AI actions', style: SwissTypography.caption.copyWith(color: mutedFg)),
          ],
        ),
      ),
    );
