...
/// VM (flutter_tester) performance harness — same 7 segments and
/// measurements as the profile-drive integration harness, runnable on
/// machines without a mobile/desktop device. Real RSS via dart:io;
/// frame timings come from SchedulerBinding. Drives the app's heaviest
/// screens (ambient background, glass nav, glass notebook workspace, a
/// blurred modal, the premium paywall, and the podcast player +
/// transcript scroll) while recording per-segment frame build/raster
/// stats and resident-set memory deltas.
///
/// Run:
///   flutter test test/perf_profile_vm_test.dart
void main() {
  // Live binding (must init before testWidgets): pumped frames go through
  // the real frame pipeline so SchedulerBinding reports engine frame
  // timings instead of "no frames captured".
  ...
  testWidgets('profile the heavy glass + animated screens', (tester) async {
    ...
    // 2. Tablet surface: the floating glass rail (720×900 logical @ DPR 2).
    ...
    // 3–7. Phone surface: 390×844 logical @ DPR 3, like the widget suite.
